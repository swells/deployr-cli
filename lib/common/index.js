...
common.checkVersion = function (callback) {
  ...
  //
  // Check the GitHub tags for `di` to see if the current
  ...
  request({
    uri: 'http://registry.npmjs.org/di/latest',
    timeout: 1000
    ...
    if (!responded) {
      ...
      try {
        ...
        if (semver.gt(pkg.version, di.version)) {
          di.log.warn('A newer version of ' + 'di'.magenta + ' is available. ' + 'please update immediately');
          di.log.help('To install the latest ' + 'di'.magenta + ' type `[sudo] npm install di -g`');
          di.log.warn('To ' + 'continue'.bold + ' without an update ' + 'type'.cyan  + ' ' + '\'yes\''.magenta);

          di.prompt.confirm('Continue without updating? Bad things might happen (no)', function (err, result) {
