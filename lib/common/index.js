...
common.checkVersion = function (callback) {
  ...
  //
  // Check the GitHub tags for `deployr` to see if the current
  // version is outdated. If it is not make sure to message the user at the end.
  //

  console.log('sesding request to check npm version...')
  request({
    uri: 'http://registry.npmjs.org/deployr/latest',
    timeout: 1000
    ...
    if (!responded) {
      ...
      try {
        ...
        if (semver.gt(pkg.version, deployr.version)) {
          deployr.log.warn('A newer version of ' + 'deployr'.magenta + ' is available. ' + 'please update immediately');
          deployr.log.help('To install the latest ' + 'deployr'.magenta + ' type `[sudo] npm install deployr -g`');
          deployr.log.warn('To ' + 'continue'.bold + ' without an update ' + 'type'.cyan  + ' ' + '\'yes\''.magenta);

          deployr.prompt.confirm('Continue without updating? Bad things might happen (no)', function (err, result) {
